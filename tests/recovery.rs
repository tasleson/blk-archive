...
fn blk_archive_pack(archive: &BlkArchive, input: PathBuf) -> Result<Child> {
    const BLK_ARCHIVE_BIN: &str = env!("CARGO_BIN_EXE_blk-archive");
    Ok(StdCommand::new(BLK_ARCHIVE_BIN)
        .arg("pack")
        .arg("-a")
...
fn test_interrupt_after_checkpoint() -> Result<()> {
    ...
    let child = blk_archive_pack(&archive, input.clone())?;
    ...
    let child = blk_archive_pack(&archive, input)?;
    let output = child.wait_with_output()?;
