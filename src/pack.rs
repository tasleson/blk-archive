    ...
    fn handle_data(&mut self, iov: &IoVec) -> Result<()> {
        ...
        if let Some(first_byte) = all_same(iov) {
            ...
            match self.transport {
                Tp::Local(ref mut da) => {
                    let ((slab, offset), len_written, hash_written) = da.data_add(h, iov, len)?;
                    self.enqueue_entry(
                        MapEntry::Data {
                            slab,
                            offset,
...
impl Packer {
    ...
    fn pack(mut self, server_addr: Option<String>) -> Result<()> {
        ...
        let ratio = (handler.stats.mapped_size as f64)
            / ((handler.stats.written + handler.stats.hashes_written + handler.stats.stream_written)
                as f64);
        ...
        if self.output.json {
            ...
            self.output.report.info(&format!(
                ...
                Size(
                    total_read
                        .saturating_sub(handler.stats.written)
