...
use crate::stream;
use crate::stream::*;
use crate::stream_archive::MemorySlab;
...
impl<D: UnpackDest> Unpacker<D, MultiFile, MemorySlab> {
    fn new(archive_dir: &Path, stream: &str, cache_nr_entries: usize, dest: D) -> Result<Self> {
        ...
        let stream_file = StreamArchive::open_read(archive_dir)?.get_stream(s_id)?;
